// app.js - Peluquería Rosa (con X para cerrar menú y número actualizado)
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    initNavigation();       // ahora incluye el botón de cierre
    initCategoryFilter();
    initSearch();
    initAuthModal();
    initScrollEffects();
...
function updateAuthUI(loggedIn, isAdmin) {
    ...
    if (loggedIn && currentUser) {
        ...
        document.querySelectorAll('#comprarNavBtn, .btn-primary[href="#catalogo"], .catalogo-footer .btn-primary').forEach(btn => {
            btn.onclick = (e) => {
                e.preventDefault();
                window.open('https://wa.me/5355555555?text=Hola,%20quiero%20comprar%20en%20Peluquería%20Rosa', '_blank');
            };
        });
    } else {
...
// ========== RESERVAS (con prefijo +53 y número de WhatsApp actualizado) ==========
